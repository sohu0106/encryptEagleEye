package com.mindmac.encryptEagleeye;
...
public class MethodParser {
	...
	private static String parseArrayArg_bp( byte[] argArray )
	{
	...
	private static ArrayList<byte[]> extractData(byte[] bytes){
...
//		return parseArrayArg(argValueList);
		...
		ArrayList<byte[]> plainByteList = extractData( argArray );
		String argValueStr = "";
		int size = plainByteList.size();
		
		for(int i=0; i<size; i++)
		{
			String plainText = Util.toHex(plainByteList.get(i));
		
			argValueStr = argValueStr + String.format("{ %s },", plainText);
		}
		return argValueStr;
	}
